...
func init() {
	...
	UserMySQLDB, err = sql.Open("mysql", dsn)
	if err != nil {
		panic(err)
	}

	UserMySQLDB.SetConnMaxIdleTime(1 * time.Hour)
	UserMySQLDB.SetConnMaxLifetime(1 * time.Hour)
	UserMySQLDB.SetMaxIdleConns(10)
	UserMySQLDB.SetMaxOpenConns(5)
}
